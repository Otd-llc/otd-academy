// Board "definition of done" panel for the revision detail page (parent plan
// WS4, advisory-first). Renders the single "Board ready" verdict bar plus the
// per-check breakdown so an author sees exactly what's left before authoring a
// guide around the board. Presentational — the page computes the readiness.
// Mirrors src/components/guide/ReadinessPanel.tsx.

import type { BoardReadiness, BoardReadinessTier } from "@/lib/board-readiness";
...
const TIER_TAG: Record<BoardReadinessTier, string> = {
  required: "",
  info: "info",
};

export function BoardReadinessPanel({
  readiness,
}: {
    ...
    <section className="mb-8 border border-panel-border bg-deep-space p-5">
      ...
      <div className="mt-4">
        <Bar
          label="Board ready"
          ok={readiness.ready}
          hint="De-risked before authoring — design validated · BOM sourced & frozen"
        />
      </div>
