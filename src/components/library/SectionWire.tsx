...
const SQ = (cycles: number, cls: string, hiW: number, loW: number, h = 8): Sym => ({ w: cycles * (hiW + loW), draw: (c) => { const w = cycles * (hiW + loW); const x = c - w / 2; const top = 13 - h, bot = 13 + h; const pts = [`${x},13`, `${x},${top}`]; let cx = x; for (let i = 0; i < cycles; i++) { pts.push(`${cx + hiW},${top}`, `${cx + hiW},${bot}`, `${cx + hiW + loW},${bot}`, `${cx + hiW + loW},${top}`); cx += hiW + loW; } pts.push(`${x + w},13`); return <polyline className={cls} points={pts.join(" ")} />; } });
