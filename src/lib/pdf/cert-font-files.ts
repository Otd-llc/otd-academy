...
// The pre-rendered embossed gold-foil seal (generated by scripts/gen-seal.ts).
// Embedded as a data URI so both renderers show the identical, genuinely
// embossed seal (satori/react-pdf can't draw metallic relief).
const SEAL = path.join(process.cwd(), "src/lib/pdf/seal.png");
let sealUri: string | null = null;
export function sealDataUri(): string {
  if (!sealUri) sealUri = `data:image/png;base64,${readFileSync(SEAL).toString("base64")}`;
  return sealUri;
}
