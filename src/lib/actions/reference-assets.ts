...
// Admin actions for the board's VERIFIED REFERENCE GERBERS — the proven Gerber
// set a learner can download to order the exact board instead of betting on
// their own export. Stored as a GERBER_ZIP Artifact on the project's PUBLISHED
// (reference) revision and served read-only by getReferenceFilesUrl().
//
...
// hard-refuses frozen revisions. But verified gerbers are admin-curated
...
// requireAdmin, GERBER_ZIP subkind only, published-revision target only. They do
// NOT touch the learner work-gate or any build; they only add/replace a
// reference download. Re-upload supersedes (the resolver serves the newest), so
// "update" is just another upload.
import { z } from "zod";
...
const REF_STAGE = "DRC_GERBER"; // GERBER_ZIP's home stage (revision-allowed)
...
/** Presign a PUT for a reference-gerber upload onto the published revision. */
export async function createReferenceGerberUploadUrl(input: unknown) {
  ...
  const cuid = createId();
  const key = artifactKey({ kind: "revision", id: revisionId }, REF_STAGE, cuid, data.filename);
...
/** Record the uploaded reference gerbers as a GERBER_ZIP artifact (freeze-exempt). */
export async function recordReferenceGerber(input: unknown) {
