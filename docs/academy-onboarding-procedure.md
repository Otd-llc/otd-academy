...
| Activation gate | Learner advance OUT of `DRC_GERBER` = the stage quiz plus a DRC report that parses to zero errors, server-side (`validateDrcReport`). **Not** "valid gerbers": no gerber is uploaded or inspected on the learner path, and the DRC check counts errors without binding the report to this board, so a clean report from an unrelated project satisfies it. | `src/lib/actions/enrollment.ts` |
