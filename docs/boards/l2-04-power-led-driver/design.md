...
| 33 | LED thermal — with HS1 | 882-100AB: **60 °C rise at 9 W = 6.7 °C/W** natural convection *(the 882-series table's rows are offset in the PDF's text layer, so this is the **worse** of the two readings the extraction permits for this height — the other is 48 °C at 9 W = 5.3 °C/W. The conclusion holds under either, so the ambiguity is not load-bearing; B1 measures it anyway)*; **derated a further 2×** to 13.4 °C/W because natural convection is markedly worse at 1.4 W than at the 9 W it is specified at; + ~1.5 °C/W interface | `T_c = 40 + 1.41 × 14.9 =` **61.1 °C** ✓ | `T_j = T_c + 8 °C/W × 1.41 =` **72.4 °C** vs T_j max **135 °C** — **63 °C of margin**, and 24 °C on the binding case limit |
