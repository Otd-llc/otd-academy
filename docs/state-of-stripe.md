...
granted/revoked correctly. What's missing is the **human-facing + ops layer**
(self-service portal, subscribe UI, dunning, admin views, reporting) and two
**activation** steps to actually start selling.

| Layer | Status |
| --- | --- |
| Schema / data model | ✅ on PROD |
...
| Subscription price provisioning | 🔜 script ready, not run |
| Course/Pass **buyable** (content published) | ⬜ blocked on content |
| Customer portal / subscribe UI / dunning / admin views / reporting | ⬜ phase 3 |
...
| Subscription | ⬜ not provisioned | ✅ | **Run `set-subscription-price.ts` (live)** + build the subscribe UI (phase 3) |
...
Phases 1–2 recorded every dollar and act on access. Phase 3 makes it a *product*
people can see and manage. None of it blocks money moving today.

**Subscriber-facing (highest value for subscriptions):**
- ⬜ **Customer billing portal** — Stripe Customer Portal (`billingPortal.sessions.create`
  + a "Manage subscription" button): view invoices, update card, self-cancel.
- ⬜ **Subscribe UI** — the `createSubscriptionCheckoutSession` action exists but no
  button/page wires it. Nothing to click to subscribe.
- ⬜ **Dunning** — email/banner on `invoice.payment_failed` (retention). The webhook
  hook is already there (currently log-only).

**Admin / ops:**
- ⬜ **Billing views in the student manager** — subscriptions / invoices / refunds /
  disputes per learner (data recorded, not surfaced).
- ⬜ **Revenue reporting** — MRR, revenue, refund rate, dispute rate from the
  Purchase/Invoice/Subscription tables.

**Compliance / polish (if selling broadly):**
...
**Suggested first slice:** billing portal + subscribe UI + dunning — the three that
turn subscriptions from "recorded in the backend" into a real self-service product.
