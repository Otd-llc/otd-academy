...
const LESSONS: Lesson[] = [
  // ── 0. what-is-a-microcontroller ─────────────────────────────────────────
  {
    ...
    contentBlocks: [
      ...
      { type: "quiz", questions: [
        ...
        { q: "A microcontroller is best described as what?", options: ["A whole small computer on one chip", "A type of resistor", "A power supply for a laptop"], answer: 0, explain: "It is a CPU, memory, and peripherals integrated on a single chip." },
      ] },
  ...
  // ── 1. esp32-gpio ────────────────────────────────────────────────────────
  {
    ...
    contentBlocks: [
      ...
      { type: "quiz", questions: [
        { q: "Before you read or drive a GPIO pin, what must you set?", options: ["Its color", "Its resistance in ohms", "Its direction: input or output"], answer: 2, explain: "A GPIO is general-purpose; you configure it as an input or an output first, then read or write it." },
  ...
  // ── 2. esp32-adc ─────────────────────────────────────────────────────────
  {
    ...
    contentBlocks: [
      ...
      { type: "prose", md: "The reference voltage is the input that reads full scale, the very top step. The ESP32's raw ADC range is small, so it offers attenuation settings that scale the incoming voltage down first, which extends the usable input range to roughly `0` to `3.1 V` at the highest attenuation, close to the rail. Pick the attenuation for the voltage you expect to measure, and the reading uses the whole range instead of a sliver of it." },
      { type: "sourceRef", label: "Espressif. ESP-IDF Programming Guide: ESP32-S3 Analog to Digital Converter (input range, attenuation).", href: "https://docs.espressif.com/projects/esp-idf/en/stable/esp32s3/api-reference/peripherals/adc.html" },
      ...
      { type: "sourceRef", label: "SparkFun. Analog to Digital Conversion tutorial (resolution, reference, quantization).", href: "https://learn.sparkfun.com/tutorials/analog-to-digital-conversion" },
      { type: "quiz", questions: [
        { q: "What sets the top of an ADC's input range?", options: ["The number of bits it has", "Its reference voltage, and on the ESP32 the attenuation setting", "The length of the wire to the sensor"], answer: 1, explain: "The reference (scaled by the ESP32's attenuation) sets full scale; the bit count sets the fineness, not the top." },
  ...
  // ── 3. esp32-pwm ─────────────────────────────────────────────────────────
  {
    ...
    contentBlocks: [
      ...
      { type: "quiz", questions: [
        { q: "What sets a PWM output's average level?", options: ["Its duty cycle, the on-fraction of each period", "The color of the wire", "The switching frequency alone"], answer: 0, explain: "The average tracks the duty cycle; the frequency only has to be fast enough that the load sees the average." },
        ...
        { q: "A hobby servo's position is set by what kind of PWM signal?", options: ["A specific frequency", "A specific pulse width", "A specific voltage"], answer: 1, explain: "Servos read the width of the high pulse (roughly 1 to 2 ms), not the duty or voltage." },
      ] },
  ...
  // ── 6. mcu-clocks-and-timers ─────────────────────────────────────────────
  {
    ...
    contentBlocks: [
      ...
      { type: "heading", text: "Periodic events, not busy-waiting" },
  ...
  // ── 7. mcu-interrupts ────────────────────────────────────────────────────
  {
    ...
    contentBlocks: [
      ...
      { type: "sourceRef", label: "SparkFun. Processor Interrupts tutorial (interrupts vs polling, ISRs).", href: "https://learn.sparkfun.com/tutorials/processor-interrupts-with-arduino" },
  ...
  // ── 8. mcu-comms-peripherals ─────────────────────────────────────────────
  {
    ...
    contentBlocks: [
      ...
      { type: "prose", md: "The ESP32 does not hard-wire each peripheral to fixed pins. A pin mux, short for multiplexer, lets you route a peripheral's signals out to almost any GPIO you choose, so you pick pins that suit your board layout and then assign the bus to them in firmware. Watch the usual caveats when you choose: keep a bus off the strapping pins and off any input-only pins." },
  ...
  // ── 9. esp32-sleep-modes ─────────────────────────────────────────────────
  {
    ...
    contentBlocks: [
      ...
      { type: "prose", md: "When a board is awake for a few milliseconds and asleep for seconds at a stretch, the average current is set almost entirely by the sleep current, not the active current. Halving the deep-sleep current can nearly double the runtime, while shaving the active current barely moves the needle. That is why low-power design is mostly about sleeping deeper and more often, not running the CPU slower." },
      ...
      { type: "quiz", questions: [
        { q: "What mostly determines a battery board's life if it sleeps most of the time?", options: ["Its sleep (idle) current", "Its peak active current alone", "The color of its status LED"], answer: 0, explain: "With the board asleep most of the time, the average current, and so the runtime, is set by the sleep current." },
  ...
  // ── 10. esp32-pinout ─────────────────────────────────────────────────────
  {
    ...
    contentBlocks: [
      ...
      { type: "prose", md: "Several kinds need care. Input-only pins can read a signal but can never drive an output. Strapping pins must be free at reset, or the board will not boot (an earlier lesson in this cluster). The native USB pins are spoken for if you use USB. Only some pins reach the ADC. And the power and ground pins are not I/O at all. Everything else is a general-purpose pin you can assign freely." },
      { type: "heading", text: "A which-pin-for-which-job checklist" },
      { type: "prose", md: "Put an output on a normal GPIO, never on an input-only pin. Keep hard pulls and always-driven signals off the strapping pins. Land an analog input on an ADC-capable pin, and prefer the `ADC1` pins so a running Wi-Fi radio does not clash with your reading. Leave the USB pins alone if you need USB. Then confirm each choice against the pinout one more time before you route the board." },
      { type: "sourceRef", label: "Espressif. ESP32-S3-DevKitC-1 User Guide (a real board's pin layout and functions).", href: "https://docs.espressif.com/projects/esp-idf/en/stable/esp32s3/hw-reference/esp32s3/user-guide-devkitc-1.html" },
      { type: "deepDive", summary: "Why ADC2 fights the Wi-Fi radio", body: "The ESP32 has two ADC blocks, and one of them, `ADC2`, is shared with the Wi-Fi driver. When Wi-Fi is running, an `ADC2` reading can fail or be made to wait, so a sensor sampled on an ADC2 pin returns garbage or nothing exactly when the board is connected. The fix is not code, it is pin choice: put any analog input that has to work while connected on an `ADC1` pin. It is a classic 'why is my sensor fine on the bench but broken over Wi-Fi' trap, and reading the pinout up front avoids it. (Espressif ESP-IDF ADC)" },
      { type: "image", src: "/guide-diagrams/mcu-pinout-map.svg", alt: "An annotated ESP32-S3 pin map with the strapping pins, ADC-capable pins, USB pair, and input-only pins each flagged with their own marker.", caption: "The pinout as a map: strapping, ADC, USB, and input-only pins each flagged before you wire." },
      { type: "callout", severity: "info", label: "Read the pinout before you route, not after", body: "Two minutes with the pin map before you assign signals saves a board re-spin. On your schematic, flag the strapping pins, the input-only pins, the USB pair, and the `ADC1` pins first, then wire your signals to what is left. It turns pin selection from a source of subtle bugs into a checklist." },
      { type: "quiz", questions: [
        ...
        { q: "Why prefer an ADC1 pin for an analog input on a Wi-Fi board?", options: ["ADC1 pins are easier to solder", "The other ADC block is shared with Wi-Fi and can fail while connected", "ADC1 always has more bits of resolution"], answer: 1, explain: "ADC2 is shared with the Wi-Fi driver, so readings on it can fail while connected; ADC1 avoids the clash." },
      ] },
