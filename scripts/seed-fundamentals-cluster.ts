...
const LESSONS: Lesson[] = [
  ...
  // ── 1. voltage-current-resistance ────────────────────────────────────────
  {
    ...
    contentBlocks: [
      ...
      { type: "deepDive", summary: "Sacrificial fuses vs resettable ones", body: "Because the current is the same everywhere in a series loop, one fuse guards the whole chain, but there are two kinds. A standard fuse is sacrificial: it blows once on an overcurrent and you replace it. A resettable fuse, a PTC or polyfuse, instead jumps to a high resistance when it overheats under a fault, choking the current down to a trickle, and then relaxes back to normal once the fault clears and power is cycled. On a bench, where a probe slip shorts things often, a PTC lets you find the mistake and carry on instead of hunting for a spare fuse. (Littelfuse PolySwitch)" },
      { type: "sourceRef", label: "Littelfuse. PolySwitch resettable PPTC devices (trip and reset behavior).", href: "https://www.littelfuse.com/products/fuses-overcurrent-protection/polyswitch-resettable-pptc-devices" },
  ...
  // ── 5. voltage-dividers ──────────────────────────────────────────────────
  {
    ...
    contentBlocks: [
      ...
      { type: "deepDive", summary: "Cleaning up the reading in firmware", body: "Real resistors carry a tolerance and the ADC itself has an offset, so the raw number a divider hands the chip is close, not exact. Two cheap firmware habits close the gap. First, average: read the pin several times in a row and take the mean, and the random jitter shrinks. Second, calibrate: measure the known error once against a trusted meter, store it as a fixed offset, and subtract it from every future reading. The hardware sets the ballpark, and a few lines of firmware make it accurate. Espressif's ESP-IDF ships an ADC calibration API for exactly this." },
  ...
  // ── 8. reactive-and-filtering ────────────────────────────────────────────
  {
    ...
    contentBlocks: [
      ...
      { type: "prose", md: "A mechanical picture makes the roll-off intuitive: an RC low-pass behaves like a shock absorber. A fast, sharp jolt gets soaked up and smoothed, while a slow, steady change passes straight through unaltered. Fast is above the cutoff and gets damped; slow is below it and gets through." },
  ...
  // ── 11. reading-a-datasheet ──────────────────────────────────────────────
  {
    ...
    contentBlocks: [
      ...
      { type: "callout", severity: "warn", label: "Before you power a fresh board: check it cold", body: "A brand-new board can hide a solder bridge that shorts two rails together, and the first thing that finds it should be your multimeter, not your laptop's USB port. Run a quick continuity check on the unpowered board before power goes anywhere near it." },
      { type: "steps", ordered: true, items: [
        "Look it over. Scan every joint under good light for bridges, especially between the close pins of the regulator and the microcontroller.",
        "Probe for shorts with the meter in continuity mode, board unpowered. Check ground to the `5 V` rail, ground to the `3.3 V` rail, and `5 V` to `3.3 V`. A beep on any of those is a short to fix before power.",
        "Confirm polarity and orientation. Check the input connector's polarity and that the regulator and any diodes face the right way.",
        "Only then power it, ideally from a current-limited supply, and confirm the `3.3 V` rail reads right before trusting anything downstream.",
      ] },
