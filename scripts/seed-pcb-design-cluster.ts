...
const LESSONS: Lesson[] = [
  ...
  // ── 1. pcb-footprints ─────────────────────────────────────────────────────
  {
    ...
    contentBlocks: [
      ...
      { type: "prose", md: "Every part datasheet includes a recommended land pattern, the manufacturer's own drawing of the pad sizes and spacing that part wants. The AP2112K regulator's datasheet gives the exact `SOT-23-5` land pattern for its package (Diodes AP2112 datasheet). Industry land-pattern geometry follows the IPC-7351 standard, but the datasheet is the first place you check for a specific part." },
      { type: "sourceRef", label: "Diodes Incorporated. AP2112 600mA CMOS LDO Regulator datasheet (recommended SOT-23-5 land pattern and package drawing).", href: "https://www.diodes.com/assets/Datasheets/AP2112.pdf" },
  ...
  // ── 4. pcb-ground-planes ──────────────────────────────────────────────────
  {
    ...
    seoDescription:
      "On a real board, ground is a filled copper plane, not a trace. A plane gives every return a low-inductance path under its signal. Pour it, keep it unbroken, and stitch it with vias.",
    clusterOrdinal: 4,
    contentBlocks: [
      { type: "prose", md: "On a real board, ground is not a trace. It is a filled sheet of copper, a plane, that covers a whole layer. A plane gives every signal's return current a low-inductance path directly beneath it, which is what keeps a board quiet. Pour a ground plane, keep it unbroken, and stitch it with vias." },
      { type: "heading", text: "Why a plane, not a trace" },
  ...
  // ── 6. pcb-drc ────────────────────────────────────────────────────────────
  {
    ...
    contentBlocks: [
      ...
      { type: "callout", severity: "critical", label: "A clean DRC is a gate, not a suggestion", body: "The courses here treat `DRC = 0` the way they treat `ERC = 0` on the schematic: a hard gate you clear before the board is allowed to move forward. An unrouted net or a clearance violation that ships is a board that does not work or cannot be built. Clear every marker, then export." },
