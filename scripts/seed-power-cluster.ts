...
const LESSONS: Lesson[] = [
  // ── 0. power-budget ──────────────────────────────────────────────────────
  {
    ...
    contentBlocks: [
      ...
      { type: "quiz", questions: [
        ...
        { q: "Why add margin to a power budget?", options: ["Because a bigger supply is always more efficient", "To cover forgotten parts, overlapping peaks, and aging", "Because supplies are sold only in fixed sizes"], answer: 1, explain: "Margin, often 20 to 30 percent, absorbs the real-world excess a bare total misses." },
      ] },
  ...
  // ── 1. batteries-101 ─────────────────────────────────────────────────────
  {
    ...
    contentBlocks: [
      ...
      { type: "prose", md: "A cell's voltage drifts as it drains. A single Li-ion cell reads about `4.2 V` full, sits near its `3.7 V` nominal for most of the discharge, and is empty around `3.0 V`. The nominal figure is the one printed on the cell and the one used for the energy math." },
      ...
      { type: "quiz", questions: [
        ...
        { q: "What does a watt-hours figure add over milliamp-hours?", options: ["The voltage, so cells of different voltages compare fairly", "The cell's self-discharge rate", "The number of charge cycles"], answer: 0, explain: "Wh = charge x voltage, an energy figure that is fair across different voltages." },
  ...
  // ── 2. lithium-battery-safety ────────────────────────────────────────────
  {
    ...
    contentBlocks: [
      ...
      { type: "prose", md: "`4.2 V` is the hard ceiling for a standard single cell, and `3.0 V` is the empty mark. Above the ceiling the chemistry breaks down and can go into thermal runaway. Drain it well below empty and the cell is damaged and may short internally on the next charge. A charger and a protection circuit exist to keep the cell inside that band." },
      { type: "sourceRef", label: "Battery University. BU-409: Charging Lithium-ion (the 4.20V/cell charge ceiling).", href: "https://batteryuniversity.com/article/bu-409-charging-lithium-ion" },
      { type: "heading", text: "Over-charge, over-discharge, and short" },
      { type: "prose", md: "Three faults do the damage. Over-charge pushes the voltage past the ceiling and overheats the cell. Over-discharge drops it below empty and ruins it. A short circuit dumps enormous current in an instant, and a lithium cell can deliver hundreds of amps into a dead short. Each fault is exactly what the protection circuit watches for." },
  ...
  // ── 3. battery-charging ──────────────────────────────────────────────────
  {
    ...
    contentBlocks: [
      ...
      { type: "quiz", questions: [
        { q: "What are the two main phases of lithium charging?", options: ["Constant voltage, then constant current", "Constant current, then constant voltage", "Trickle charge, then fast charge"], answer: 1, explain: "CC lifts the voltage to the ceiling; CV then holds it while the current tapers." },
        { q: "Charging ends when what happens in the constant-voltage phase?", options: ["The current falls below a small termination threshold", "A fixed timer runs out", "The cell reaches 5 V"], answer: 0, explain: "Termination is by current, a few percent of the cell's rating, not by a clock." },
  ...
  // ── 4. linear-regulators-ldo ─────────────────────────────────────────────
  {
    ...
    contentBlocks: [
      ...
      { type: "deepDive", summary: "When an LDO is exactly the right part", body: "An LDO is the correct choice in three cases. When the drop is small, the wasted power is small, so a `3.3 V` LDO from a `3.7 V` cell is efficient. When the current is low, even a larger drop makes little heat. And when a rail feeds noise-sensitive analog parts, an LDO's quiet, ripple-free output beats a switching regulator's chopped one. Reach for a switcher when the drop is large and the current is high; reach for an LDO when the drop is small, the current is low, or the rail must be clean." },
      ...
      { type: "quiz", questions: [
        { q: "What limits how much an LDO can step down at high current?", options: ["The heat from (Vin minus Vout) times the current", "The value of its output capacitor", "Its dropout voltage alone, no matter the current"], answer: 0, explain: "The dropped voltage times the current becomes heat, and the thermal ceiling is the real limit." },
        { q: "What is an LDO's dropout voltage?", options: ["The output voltage it produces", "The minimum input-to-output gap it needs to keep regulating", "The maximum current it can pass"], answer: 1, explain: "Below the dropout the output falls out of regulation and follows the input down." },
        { q: "When is an LDO a good choice?", options: ["A large drop at high current", "A small drop, low current, or a rail that must be quiet", "Whenever efficiency is the top priority"], answer: 1, explain: "Small drop, low current, or a noise-sensitive rail: those favor an LDO over a switcher." },
      ] },
      { type: "sourceRef", label: "Prerequisite: power and heat", href: "/library/power-and-heat" },
  ...
  // ── 5. buck-converters ───────────────────────────────────────────────────
  {
    ...
    contentBlocks: [
      ...
      { type: "heading", text: "How switching avoids the heat" },
      ...
      { type: "quiz", questions: [
        { q: "Why is a buck more efficient than an LDO for a big step-down?", options: ["It sheds the wasted power through a bigger heatsink", "It switches and filters instead of burning the difference as heat", "It drops the extra voltage across a larger resistor"], answer: 1, explain: "A buck stores and transfers energy through an inductor rather than dropping it across a pass element." },
        { q: "Which two parts smooth a buck's chopped input into a steady output?", options: ["An inductor and a capacitor", "Two resistors", "A transformer and a diode"], answer: 0, explain: "The inductor stores energy and the output capacitor holds the voltage steady between pulses." },
        { q: "In a buck, the duty cycle sets what?", options: ["The output ripple amplitude only", "The switching frequency only", "The output as a fraction of the input"], answer: 2, explain: "Vout = D x Vin: the on-fraction of each cycle sets the output level." },
      ] },
  ...
  // ── 6. boost-converters ──────────────────────────────────────────────────
  {
    ...
    contentBlocks: [
      ...
      { type: "quiz", questions: [
        { q: "What does a boost regulator let a `3.7 V` cell do?", options: ["Power a `1.8 V` chip more efficiently", "Drive a higher rail, such as `5 V`", "Recharge without a charger IC"], answer: 1, explain: "A boost steps the cell's voltage up so it can feed a rail above its own voltage." },
        ...
        { q: "Which part stores the energy that a boost releases at a higher voltage?", options: ["The output capacitor", "The Schottky diode", "The inductor"], answer: 2, explain: "The inductor builds current while the switch is closed and releases it forward when the switch opens." },
      ] },
  ...
  // ── 7. ldo-vs-switching-regulator ────────────────────────────────────────
  {
    ...
    contentBlocks: [
      ...
      { type: "quiz", questions: [
        { q: "You need a quiet `3.3 V` analog rail from a `3.6 V` input. Which regulator?", options: ["A boost", "A high-power buck", "An LDO, for the small drop and low noise"], answer: 2, explain: "A small drop and a noise-sensitive rail is exactly where an LDO wins." },
  ...
  // ── 8. reverse-polarity-protection ───────────────────────────────────────
  {
    ...
    contentBlocks: [
      ...
      { type: "quiz", questions: [
        { q: "Why prefer a P-channel MOSFET ideal diode over a series diode for reverse protection?", options: ["It drops a larger voltage than the diode does", "It wastes far less power and heat than the diode's forward drop", "It raises the output voltage"], answer: 1, explain: "The MOSFET's milliohm on-resistance drops almost nothing, unlike the diode's steady forward-voltage loss." },
        { q: "What causes an inrush surge at power-on?", options: ["The empty bulk capacitors gulping current as they charge", "The regulator reaching its output voltage", "The reverse-polarity MOSFET switching on"], answer: 0, explain: "Discharged capacitors look like a near-short for an instant and pull a large charging spike." },
  ...
  // ── 9. power-sequencing ──────────────────────────────────────────────────
  {
    ...
    contentBlocks: [
      ...
      { type: "quiz", questions: [
        { q: "What problem does power sequencing avoid?", options: ["Rails coming up in a bad order, risking latch-up or a surge", "A regulator running above its rated temperature", "Excess ripple on a single rail"], answer: 0, explain: "Some chips require a rail order; violating it can latch the chip up or stack inrush surges." },
        { q: "What does soft-start do to a rail?", options: ["Clamps the rail to a lower final voltage", "Ramps it up gently instead of stepping it on instantly", "Raises it above its rated voltage"], answer: 1, explain: "The smooth ramp limits inrush and gives the rest of the board time to react." },
        { q: "A simple way to sequence rails without a dedicated chip is what?", options: ["Adding more decoupling capacitors", "Raising every rail's voltage together", "An enable chain, where one rail's power-good enables the next"], answer: 2, explain: "Chaining enables, or an RC delay on an enable pin, orders the rails by construction." },
      ] },
  ...
  // ── 10. battery-runtime ──────────────────────────────────────────────────
  {
    ...
    contentBlocks: [
      ...
      { type: "heading", text: "Average draw is what counts" },
      ...
      { type: "sourceRef", label: "Battery University. BU-503 (usable design capacity fades to 70 to 80 percent of rated).", href: "https://batteryuniversity.com/article/bu-503-how-to-calculate-battery-runtime" },
      ...
      { type: "quiz", questions: [
        { q: "What dominates the battery life of a board that is mostly asleep?", options: ["The brief active burst", "The sleep or idle current", "The battery's nominal voltage"], answer: 1, explain: "With the board asleep almost all the time, the sleep current sets the runtime." },
