...
const LESSONS: Lesson[] = [
  ...
  // ── 2. spi-bus ────────────────────────────────────────────────────────────
  {
    ...
    contentBlocks: [
      ...
      { type: "sourceRef", label: "SparkFun. Serial Peripheral Interface (SPI): the four lines, modes, and slave-select.", href: "https://learn.sparkfun.com/tutorials/serial-peripheral-interface-spi" },
      { type: "heading", text: "One chip-select per peripheral" },
      { type: "prose", md: "The controller talks to exactly one peripheral at a time by pulling that peripheral's `CS` low; the rest ignore the bus while their `CS` stays high. Add a second peripheral and you add one more `CS` pin. That is the main cost of SPI: the speed is cheap, the pins are not." },
  ...
  // ── 3. i2c-bus ────────────────────────────────────────────────────────────
  {
    ...
    contentBlocks: [
      ...
      { type: "sourceRef", label: "NXP. UM10204 I2C-bus specification and user manual, Rev. 7.0 (addressing, START/STOP, bus speeds).", href: "https://www.nxp.com/docs/en/user-guide/UM10204.pdf" },
  ...
  // ── 4. spi-vs-i2c-vs-uart ─────────────────────────────────────────────────
  {
    ...
    contentBlocks: [
      ...
      { type: "quiz", questions: [
        ...
        { q: "Where do you find out which bus a given sensor uses?", options: ["By measuring the idle voltage", "By counting its pins", "On the first page of its datasheet"], answer: 2, explain: "The interface is stated up front on the datasheet; the part chooses its bus, not you." },
      ] },
  ...
  // ── 5. usb-basics ─────────────────────────────────────────────────────────
  {
    ...
    contentBlocks: [
      ...
      { type: "deepDive", summary: "Why USB is more than a fast UART", body: "A UART link is two chips preset to the same speed, sending bytes with no introduction. USB detects the device's speed from resistors on the data lines, resets it, assigns it an address, and reads its capabilities before any payload moves. That negotiation is what lets one port accept a keyboard, a flash drive, and your board with no manual setup, and it is why a USB stack is a real piece of firmware rather than a shift register. On a small board you usually get it for free from the microcontroller's built-in USB, but the handshake is still happening under the hood." },
  ...
  // ── 6. usb-c-connector ────────────────────────────────────────────────────
  {
    ...
    contentBlocks: [
      ...
      { type: "sourceRef", label: "Texas Instruments. A Primer on USB Type-C and USB Power Delivery Applications (SLYY109): CC roles.", href: "https://www.ti.com/lit/slyy109" },
  ...
  // ── 8. pull-ups-and-pull-downs ────────────────────────────────────────────
  {
    ...
    clusterOrdinal: 8,
    contentBlocks: [
      { type: "prose", md: "A digital input wired to nothing does not read a steady 0. It floats, picking up noise and flickering between high and low. A pull-up or pull-down resistor fixes that by tying the line to a known default level, and getting that default wrong is a classic reason a bus sits dead." },
  ...
  // ── 10. debugging-a-bus ───────────────────────────────────────────────────
  {
    ...
    contentBlocks: [
      ...
      { type: "prose", md: "Match the symptom to the bus. On I2C, no pull-ups means both lines stay low and nothing moves, and two devices at the same address answer at once and collide. On SPI, the wrong mode (CPOL/CPHA) makes every byte read wrong even though the wiring is right. On UART, a baud mismatch prints garbage, and swapped `TX` and `RX` means nothing arrives at all." },
      ...
      { type: "callout", severity: "info", label: "When a bus is silent, in this order", body: "Work from the most common cause outward, so you spend a probe on the likely fault, not the rare one." },
