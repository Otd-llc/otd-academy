...
- **Hex Cluster — an open hardware standard, distributed.** A modular hex tile system published CC BY 4.0 at `/hex`, with a browser configurator (a separate deploy) that lays out a build and hands back a printable download. It is a reach asset rather than a revenue one: a mounting standard is worth more adopted than sold. The download itself is where most of the engineering went — see [Notable engineering](#notable-engineering).
...
- **A downloaded 3MF carries its own print settings — in three slicer dialects, from one table.** The Hex Cluster download (`/hex`) is a *build*, not a zip of parts: pick parts, quantities and bed size, and the endpoint packs them onto plates, one object per instance, named and seated at `z = 0`. The intent then travels **inside** the file, because alpha feedback was not "the README could be clearer" — it was that nobody read it and nobody selected the infill, and gyroid on these parts is a **torsion requirement** rather than a preference. Every object carries per-object settings for the Orca family (`Metadata/model_settings.config`), PrusaSlicer (`Metadata/Slic3r_PE_model.config`) and Cura (a `cura:`-namespaced `<metadatagroup>` inside `3D/3dmodel.model`), all derived from one [`PRINT_INTENT_TABLE`](src/lib/hex-print-intent.ts) — the same three facts previously existed in five hand-typed copies and had already drifted.

  The dialects are **transforms, not renames**, and each difference is a way to be silently wrong: infill density is `30%` for Orca but bare `30` for Cura, which types it as a float, while PrusaSlicer multiplies a bare `30` into **3000%**. A bad *enum* never fails — `"Gyroid"` is quietly replaced with **grid** — so a build-time guard refuses a table the slicer would rewrite. Cura's support threshold is measured from vertical with the sense inverted, so it is deliberately **not** carried: a wrong number is a wrong print, where its absence just leaves Cura on its own default. Which parts need support is likewise *collected from the slicer* via a calibration plate rather than computed — two homemade metrics were confidently wrong, both in the direction that reads as a clean result.

  Finally, a fail-safe for the path that defeats all of it: `File > Import` parses the config and then calls `config.reset()`, so an imported plate arrives with support off and nothing says so. Every support part therefore carries **one painted facet**, which lives on the mesh and survives that reset, making the slicer raise "Support enforcers are used but support is not enabled" against the named object. It is the most *upward*-facing triangle, because only downward-facing paint projects into the support layers — so it fires the warning and changes the sliced result by nothing, confirmed by slicing a painted plate against an unpainted twin. Every claim here was opened in the real slicers rather than inferred; three independent source readings disagreed beforehand and two were wrong.
