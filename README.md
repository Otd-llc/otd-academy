...
- **Public DB reads are a function of time, not traffic.** The SEO surface (69 library lessons across 6 clusters, the parts catalog, the courses index) is crawled far more than it is read by humans, so every uncached hit was a database read on a metered serverless Postgres. Under **Next 16 Cache Components** the user-independent loaders are `use cache` + tagged with a 1-hour window, and writes fire `updateTag` so an edit is live on the next request rather than an hour later — measured on a production build with the Prisma query log: **`/library` ×20 → 0 queries**, `/parts` 50 → 0, `/pricing` 30 → 3. Two things make this subtler than "add the directive". First the **cache key**: `use cache` keys on arguments and a `[slug]` route param matches *any* string, so an unbounded cached loader mints an entry plus a DB query per garbage URL a crawler tries — reintroducing the exact traffic-scales-with-reads behaviour it was meant to remove. Every param-taking cached function is bounded against an already-cached row set. Second, **where the entry lives**: `use cache` is in-memory by default, so what spares a cold instance a DB read is whether the entry was resolved during the build and shipped with the deployment — measured by pointing `DATABASE_URL` at a dead port, where a cold `/library` still serves all 69 lessons while `/parts`, whose cached loader only ever runs at request time, degrades to zero rows. Getting the *markup* into the prerendered shell is a second and separate step: `/library` reads the session for its signed-in overlay, which postponed the whole body and left the shell as 48 kB of chrome with **zero** lesson links, so every personalised element now sits behind its own `<Suspense>` — a Cache Components build prerenders fallbacks, and that is what put the 69 links in `library.html`. The lever, if a request-time route ever draws real crawl volume, is one directive — `'use cache: remote'`, which on Vercel maps to the Runtime Cache; it is billed, so it is deliberately off at today's traffic. Chrome is **structural** (a `(chrome)/` route group) rather than sniffed from a request header, which is what lets the header prerender into the first flush instead of streaming in above the content and shoving it down. Every cached read is bounded, tagged, and paired with an invalidator: the one shared window and every tag constant live in `src/lib/cache-profile.ts`, the invalidators in `src/lib/cache-invalidate.ts`.
...
- **Next.js 16** (App Router, RSC + client islands) · **TypeScript 5** · **React 19**. **Cache Components / PPR is on** (`cacheComponents: true`): dynamic is the default, caching is opt-in via `use cache`, and the app shell prerenders while per-user fragments stream — see [Notable engineering](#notable-engineering)
...
4. **Caching is not optional knowledge.** Cache Components is on, so a route-segment `dynamic`/`revalidate`/`runtime` export will not compile, a cached function may not read the session, and vitest cannot see cache behaviour at all. Before touching a public read path, read `src/lib/cache-profile.ts` (the one shared window + every tag constant) and `src/lib/cache-invalidate.ts` (the invalidators): a cached read must be bounded against an already-cached row set, tagged with a constant, and paired with an invalidator that fires that tag.
